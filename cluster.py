...
from sklearn.metrics import normalized_mutual_info_score, classification_report,\
    accuracy_score, f1_score,recall_score, precision_score, adjusted_rand_score
...
def cluster(n_clusters, features, labels, count=10):
    ...
    if np.min(gt) == 1:
        gt -= 1
    acc_avg, acc_std, nmi_avg, nmi_std, ari_avg, ari_std, f1_avg, f1_std = get_avg_matric(gt, pred_all, count)

    print(f'H Kmeans ACC={acc_avg:.2f}±{acc_std:.2f}, NMI={nmi_avg:.2f}±{nmi_std:.2f},\n'
          f'ARI={ari_avg:.2f}±{ari_std:.2f}, f1={f1_avg:.2f}±{f1_std:.2f}')

    return acc_avg, acc_std, nmi_avg, nmi_std, ari_avg, ari_std, f1_avg, f1_std
